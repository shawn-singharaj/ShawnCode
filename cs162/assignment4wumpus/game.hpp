...
private:
	...
	vector <vector<room>> cave; // the 2d vector representing rooms that holds event pointers
...
public:
	 ...
	 * 		player p, the player class that can be moved using class functions
	 * Pre-condition: Player is capable of moving in this direction (without
	 * 		hitting the edge of the board)
	 * Post-condition: Player is moved accordingly.
	 */
	...
	/*
	move_wumpus
	Moves wumpus to a random empty room in the cave by deleting the current wumpus and allocating new memory for a different empty room
	*/
	void move_wumpus();
