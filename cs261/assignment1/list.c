...
struct list* list_create()
{
    ...
    linked_list->head = NULL; // head set to null
...
void list_free(struct list* list)
{
    ...
    while (itr != NULL) {
        struct node* temp = itr; // use temp node that will be freed, separate from the itr going thru the list
        itr = itr->next;
        free(temp);  
    }
    free(list); // free list
...
void list_insert(struct list* list, void* val)
{
   ...
   // set new head to parameter and set next to the old head
    new_head->val = val;        
...
void list_insert_end(struct list* list, void* val)
{
    ...
    new_tail->val = val; // set values to parameters
    new_tail->next = NULL;

    if (list->head == NULL) { // if list is empty, return
        list->head = new_tail;
        return;
    }

    struct node* itr = list->head; // traverse to end of list
    while(itr->next != NULL){
        itr = itr->next;
    }
    itr->next = new_tail; // set new tail 
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    int position = 0; 
    struct node* itr = list->head;
    struct node* prev = NULL; // set a previous node pointer to null

    while(itr != NULL){
        if(cmp(val, itr->val) == 0){ // if there is a match in the void*
            if(prev == NULL){  // if first node
                list->head = itr->next;
            }
            else{ // previous node is linked to the next
                prev->next = itr->next;
            }   
...
void list_remove_end(struct list* list)
{
	...
	struct node* itr = list->head;
    while(itr->next->next != NULL){ // traverse to end
        itr = itr->next;
    }
    free(itr->next); // free the last node and set to null
    itr->next = NULL; 
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    int position = 0;
    struct node* itr = list->head;
    while(itr != NULL){ // traverse list
        if(cmp(val, itr->val) == 0){ // if void* are equal
            return position; // return the position of node
        }
        itr = itr->next;
...
void list_reverse(struct list* list)
{
    ...
    struct node* itr = list->head; // set pointers to current, previous, and next nodes
    struct node* prev = NULL;
    struct node* next = NULL;

    while(itr != NULL){ // traverse list
        next = itr->next;  //set next pointer

        itr->next = prev; // current next switched to previous

        prev = itr; // previous set to current
        itr = next; // current is traveresed
    }
    list->head = prev;    
