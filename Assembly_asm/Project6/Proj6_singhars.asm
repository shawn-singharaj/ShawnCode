...
temp_arr	SDWORD	TEMPS_PER_DAY DUP(?)
bytesRead	DWORD	?
...
lineNum		BYTE	"Corrected input line ",0
...
; Posconditions: changes registers edi, eax, ecx, ebx, edx
; Receives:
;	[ebp + 8] = the address of the array to be printed
;	[ebp + 12] = line header string
; Returns: None 
WriteTempsReverse PROC
    ...
    sub     eax, 4                     
    add     edi, eax                   ; set edi address to last index

    mov     ebx, 0                     ; index counter
    sub     esp, 4
    mov     DWORD PTR [ebp - 4], 1     ; line counter
...
_printArray:                         
    cmp     ebx, TEMPS_PER_DAY         ; check if we reached the end
    je      _newLine                   ; jump to _newLine if done printing

    mov     eax, [edi]                 ; load temperature at index into eax
    call    writeInt                   ; print the temperature
    mDisplayChar DELIMITER             ; print delimiter after the value
    sub     edi, 4                      ; move to the previous element in the array
    inc     ebx                         ; increment counter
    jmp     _continuePrint              ; continue the loop

_newLine:
    inc     DWORD PTR [ebp - 4]        ; increment line counter
    call    crLf                       ; print newline
    mov     edx, [ebp + 12]            ; print header again
    call    writeString
    mov     eax, [ebp - 4]
    call    writeDec                   ; print the line number
    mDisplayChar COLON                 ; print a colon
    mov     ebx, 0                     ; reset array index counter
    jmp     _printArray                ; continue printing next line

_continuePrint:
    loop    _printArray                ; loop based on the value in ecx
