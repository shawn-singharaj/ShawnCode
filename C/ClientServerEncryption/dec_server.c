...
int main(int argc, char *argv[]){
    ...
    if (listenSocket < 0) {
        perror("DEC SERVER: ERROR opening socket");
        exit(1);
    }
    ...
    if(setsockopt(listenSocket, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) < 0) {
        perror("DEC SERVER: setsockopt failed");
    }
    ...
    if(bind(listenSocket, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0){
        perror("DEC SERVER: ERROR on binding");
        close(listenSocket);
        exit(1);
    }
    ...
    if(listen(listenSocket, 5) < 0) {
        perror("DEC SERVER: ERROR on listen");
        close(listenSocket);
        exit(1);
    }

    // Handle zombie processes - CRITICAL for concurrency
    ...
    while(1){
        ...
        if(connectionSocket < 0){
            perror("DEC SERVER: ERROR on accept");
            continue;
        }
        ...
        if(pid == 0){
            ...
            if(idRecv < 0) {
                fprintf(stderr, "DEC SERVER: ERROR receiving client ID\n");
                close(connectionSocket);
                exit(1);
            }

            if (strcmp(idBuffer, "dec") != 0) {
                fprintf(stderr, "DEC SERVER: Invalid client\n");
                close(connectionSocket);
                exit(2);   
            }
            ...
            if(!textBuffer) {
                perror("DEC SERVER: malloc failed");
                close(connectionSocket);
                exit(1);
            }
            memset(textBuffer, '\0', 100001);

            // Receive ciphertext (up to 100000 bytes)
            int textcharsRead = recvAll(connectionSocket, textBuffer, 100000);
            if(textcharsRead < 0){
                perror("DEC SERVER: ERROR reading ciphertext from socket");
                free(textBuffer);
                close(connectionSocket);
                exit(1);
            }
            ...
            if(actualLength == 0) {
                fprintf(stderr, "DEC SERVER: Empty ciphertext\n");
                free(textBuffer);
                close(connectionSocket);
                exit(1);
            }
            ...
            if(keycharsRead < 0){
                perror("DEC SERVER: ERROR reading key from socket");
                free(textBuffer);
                free(keyBuffer);
                close(connectionSocket);
                exit(1);
            ...
            // Decrypt - USE actualLength!
            for(int i = 0; i < actualLength; i++){
                int cipherVal;
                if (textBuffer[i] == ' ') {
                    cipherVal = 26;
                } else {
                    cipherVal = textBuffer[i] - 'A';
                }
                ...
                int decryptVal = cipherVal - keyVal;

                // Fix negative mod issue
                if (decryptVal < 0)
                    decryptVal += 27;
                ...
                if (decryptVal == 26)
                    ...
                    buffer[i] = decryptVal + 'A';
            }

            // Send decrypted text
            int sent = sendAll(connectionSocket, buffer, actualLength);
            if(sent < 0){
                perror("DEC SERVER: ERROR writing to socket");
            }
            ...
            perror("DEC SERVER: ERROR forking");
            close(connectionSocket);
        }
        else{
            close(connectionSocket);
