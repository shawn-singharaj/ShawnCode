...
struct dynarray* dynarray_create(){
    ...
    struct dynarray* dynamic_array = (struct dynarray*) malloc(sizeof(struct dynarray));

    // capacity set to 2
    dynamic_array->data = (void**)malloc((sizeof(void*) * 2));
...
int dynarray_size(struct dynarray* da){
    ...
    return da->size; // return current size
}
...
void dynarray_insert(struct dynarray* da, void* val){
    if(da->size == da->capacity) // if size is at capacity, double capacity and reallocate memory to do so
    {
        da->capacity = da->capacity * 2;
        da->data = realloc(da->data, (da->capacity * sizeof(void*)));
    }
    
    da->data[da->size] = val; // insert value to end of array, increment size
    da->size++;
...
void dynarray_remove(struct dynarray* da, int idx){
   if(idx >= da->size || idx < 0){ // if idx is invalid
    return;
   }
   else{
    for(idx; idx < da->size - 1; idx++)
      da->data[idx] = da->data[idx + 1]; // shift array from that point
   }
...
void* dynarray_get(struct dynarray* da, int idx){
    ...
    return da->data[idx]; // return the current array index
}
...
void dynarray_set(struct dynarray* da, int idx, void* val){
    if(idx < 0 || idx >= da->size) // if invalid idx
        return; 
    
    da->data[idx] = val; // sets current index to parameter
